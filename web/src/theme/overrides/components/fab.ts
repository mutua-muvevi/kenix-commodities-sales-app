...
export function fab(theme: ThemeTypes) {
	const lightMode = theme.palette.mode === "light";

	const rootStyles = (ownerState: { color: string; variant: string; size: string }) => {
		...
		const defaultStyle = {
			...
			...((outlinedVariant || outlinedExtendedVariant) && {
				...
				...((defaultColor || inheritColor) && {
					border: `solid 1px ${alpha(theme.palette.grey[500].toString(), 0.32)}`,
				}),
				...(defaultColor && {
					...(!lightMode && {
			...
			...((softVariant || softExtendedVariant) && {
				...
				...(inheritColor && {
					...
					"&:hover": {
						backgroundColor: alpha(theme.palette.grey[500].toString(), 0.24),
					},
				}),
			}),
		};
